...
class CSVSearchApp:
        ...
        try:
            ...
            if not self.found_rows:
                ...
                if visible_columns:
                    start_col, end_col = visible_columns
                    total_cols = len(self.result_tree["columns"])
                    first_col_name = self.result_tree.heading(self.result_tree["columns"][start_col])["text"]
                    last_col_name = self.result_tree.heading(self.result_tree["columns"][end_col])["text"]
                    
                    self.status_var.set(
                        f"총 {len(self.found_rows)}개 행 찾음 | "
                        f"열 보기: {start_col+1}-{end_col+1}/{total_cols} "
                        f"({first_col_name}~{last_col_name}) | "
                        f"◀▶ 키나 버튼으로 이동 | 클립보드에 복사됨"
                    )
                else:
                    self.status_var.set(f"총 {len(self.found_rows)}개의 행에서 검색어를 찾았습니다. (클립보드에 복사됨)")
    ...
    def copy_to_clipboard(self, silent=False):
        """검색 결과를 클립보드에 복사 (엑셀에 붙여넣기 가능한 형식)"""
        if not self.result_tree or not self.found_rows:
            if not silent:
                messagebox.showinfo("알림", "복사할 검색 결과가 없습니다.")
            return
        
        try:
            # 헤더 준비
            header_row = ["행"] + [self.headers[idx] for idx in self.selected_columns]
            
            # 탭으로 구분된 텍스트 생성 (엑셀 붙여넣기 형식)
            clipboard_text = "\t".join(str(item) for item in header_row) + "\n"
            
            for row in self.found_rows:
                clipboard_text += "\t".join(str(item) for item in row) + "\n"
