...
class ExcelSearchApp:
    ...
    def search(self):
        ...
        try:
            ...
            # openpyxl로 엑셀 파일 읽기
            workbook = openpyxl.load_workbook(self.excel_file, read_only=True, data_only=True)
                ...
                try:
                    ...
                    for row_idx, row in enumerate(sheet.rows, 1):
                        for col_idx, cell in enumerate(row, 1):
                            if cell.value is not None and search_term.lower() in str(cell.value).lower():
                                col_letter = get_column_letter(col_idx)
                                self.results.append((sheet_name, row_idx, col_letter, cell.value))
                except Exception as e:
                    messagebox.showwarning("경고", f"시트 '{sheet_name}' 읽는 중 오류 발생: {str(e)}")
